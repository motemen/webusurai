...
export async function getState(
  ...
  if (0 <= hours && hours < 3) {
    ...
    const brokenAtTime = await KV.get(`${locKey}:${loc[locKey]}`)
    if (!brokenAtTime) {
      return FROZEN
    }
...
async function updateStateBroken(
  ...
  if (state.state === 'FROZEN') {
    await KV.put(`${locKey}:${loc[locKey]}`, now.toString())
    return true
  }
...
export async function handleEvent(event: FetchEvent): Promise<Response> {
  ...
  if (url.pathname === '/state') {
    ...
    const result: GetStateResult = {
      states: states.filter((s): s is LocState => !!s.state),
      loc,
    }
